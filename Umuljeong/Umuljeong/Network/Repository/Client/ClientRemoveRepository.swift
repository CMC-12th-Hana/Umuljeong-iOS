...
class ClientRemoveRepository {
    func requestClientRemove(clientId: Int, completion: @escaping (Result<Bool, ResError>) -> Void) {
            ...
            case .success:
                ...
                if statusCode == 401 {
                    ...
                    ApiManager.shared.refreshToken { isSuccess in
                        if isSuccess {
                            print("토큰 새로 받아오기 성공 ><")
                            self.requestClientRemove(clientId: clientId, completion: completion)
                        } else {
