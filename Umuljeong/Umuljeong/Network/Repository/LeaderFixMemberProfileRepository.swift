//
//  LeaderFixMemberProfileRepository.swift
//  Umuljeong
//
...
class LeaderFixMemberProfileRepository {
    //서버에서 바꿔주기 전까지 유지
    func requestMemberFix(memberId:Int, name: String, phoneNumber: String, staffNum: String, staffRank: String, completion: @escaping (Result<Bool, ResError>) -> Void) {
        
        guard let accessToken = KeychainSwift().get("accessToken") else {
            return completion(.failure(.NON_TOKEN))
        }

        let url = URLConstants.Member_Leader_Fix_OtherDetail(memberId: String(memberId))
...
//            case .success(let res):
                ...
                if statusCode == 401 {
                    ...
                    ApiManager.shared.refreshToken { isSuccess in
                        if isSuccess {
                            print("토큰 새로 받아오기 성공 ><")
                            self.requestMemberFix(memberId: memberId, name: name, phoneNumber: phoneNumber, staffNum: staffNum, staffRank: staffRank, completion: completion)
                        } else {
