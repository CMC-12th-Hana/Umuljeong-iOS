//
//  MemberOtherDetailRepository.swift
//  Umuljeong
//
...
class MemberOtherDetailRepository {
    func requestOtherMemberDetail(memberId:Int, completion: @escaping (Result<UserInfoResponse?, ResError>) -> Void) {
        
        guard let accessToken = KeychainSwift().get("accessToken") else {
            return completion(.failure(.NON_TOKEN))
        }
        
        let url = URLConstants.Member_OtherDetail(memberId: String(memberId)) //통신할 API 주소
            ...
            case .success: //데이터 통신이 성공한 경우에
                ...
                if statusCode == 401 {
                    ...
                    ApiManager.shared.refreshToken { isSuccess in
                        if isSuccess {
                            print("토큰 새로 받아오기 성공 ><")
                            self.requestOtherMemberDetail(memberId: memberId, completion: completion) //재귀함수 //함수를 다시 실행
                        } else {
